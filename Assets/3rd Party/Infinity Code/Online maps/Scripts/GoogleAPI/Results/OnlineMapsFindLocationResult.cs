...
/// </summary>
public class OnlineMapsFindLocationResult
{
    /// <summary>
    /// Array containing the separate address components.
    /// </summary>
    public OnlineMapsFindLocationResultAddressComponent[] address_components;
    ...
    public OnlineMapsFindLocationResult(OnlineMapsXML node)
    {
        ...
        foreach (OnlineMapsXML n in node)
        {
            ...
            else if (n.name == "address_component") address_components.Add(new OnlineMapsFindLocationResultAddressComponent(n));
            else if (n.name == "geometry")
            {
                foreach (OnlineMapsXML gn in n)
                {
                    if (gn.name == "location") geometry_location = OnlineMapsGoogleAPIQuery.GetVector2FromNode(gn);
...
/// </summary>
public class OnlineMapsFindLocationResultAddressComponent
{
    /// <summary>
    ...
    public OnlineMapsFindLocationResultAddressComponent(OnlineMapsXML node)
    {
        List<string> types = new List<string>();
    ...
    {
        return "OnlineMapsFindLocationResultAddressComponent. Types: {" + string.Join(",", types) + "}, Long name: {" + long_name + "}, Short name: {" + short_name + "}";
    }
}