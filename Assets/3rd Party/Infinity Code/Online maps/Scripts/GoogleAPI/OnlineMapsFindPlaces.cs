...
public class OnlineMapsFindPlaces: OnlineMapsGoogleAPIQuery
{
    ...
    /// <returns>Query instance to the Google API.</returns>
    public static OnlineMapsGoogleAPIQuery FindNearby(string key, NearbyParams p)
    {
        return new OnlineMapsFindPlaces(key, p);
    }
    ...
    /// </param>
    /// <returns>Query instance to the Google API.</returns>
    public static OnlineMapsGoogleAPIQuery FindText(string query, string key, Vector2 lnglat = default(Vector2), int radius = -1, string language = null, string types = null, int minprice = -1, int maxprice = -1, bool opennow = false)
    {
        return new OnlineMapsFindPlaces(key, new TextParams
        {
            query = query,
            lnglat = lnglat,
            radius = radius,
    ...
    /// <param name="p">The object containing the request parameters.</param>
    /// <returns>Query instance to the Google API.</returns>
    public static OnlineMapsGoogleAPIQuery FindText(string key, TextParams p)
    {
        return new OnlineMapsFindPlaces(key, p);
    }
    ...
    /// </param>
    /// <returns>Query instance to the Google API.</returns>
    public static OnlineMapsGoogleAPIQuery FindRadar(Vector2 lnglat, int radius, string key, string keyword = null, string name = null, string types = null, int minprice = -1, int maxprice = -1, bool opennow = false)
    {
        return new OnlineMapsFindPlaces(key, new RadarParams(lnglat, radius)
        {
            keyword = keyword,
            name = name,
            types = types,
    ...
    /// <param name="p">The object containing the request parameters.</param>
    /// <returns>Query instance to the Google API.</returns>
    public static OnlineMapsGoogleAPIQuery FindRadar(string key, RadarParams p)
    {
        return new OnlineMapsFindPlaces(key, p);
    }
    ...
    /// </param>
    /// <returns>Array of result.</returns>
    public static OnlineMapsFindPlacesResult[] GetResults(string response, out string nextPageToken)
    {
        nextPageToken = null;
        ...
        try
        {
            ...
            List<OnlineMapsFindPlacesResult> results = new List<OnlineMapsFindPlacesResult>(resNodes.count);
            foreach (OnlineMapsXML node in resNodes) results.Add(new OnlineMapsFindPlacesResult(node));
    ...
    /// <summary>
    /// Request parameters to Nearby Search
    /// </summary>
    public class NearbyParams:RequestParams
    {
        ...
        /// </summary>
        public OnlineMapsFindPlacesRankBy rankBy = OnlineMapsFindPlacesRankBy.prominence;
        ...
        public override void AppendParams(StringBuilder url)
        {
            ...
            if (minprice != -1) url.Append("&minprice=").Append(minprice);
            if (maxprice != -1) url.Append("&maxprice=").Append(maxprice);
            if (opennow) url.Append("&opennow");
            if (rankBy == OnlineMapsFindPlacesRankBy.distance) url.Append("&rankby=distance");
            if (!string.IsNullOrEmpty(pagetoken)) url.Append("&pagetoken=").Append(pagetoken);
            if (zagatselected) url.Append("&zagatselected");
        }
    }

    /// <summary>
    /// Request parameters to Text Search
    /// </summary>
    ...
    /// <summary>
    /// Request parameters to Radar Search
    /// </summary>
